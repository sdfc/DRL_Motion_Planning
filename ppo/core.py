...
class MLPOrnsteinUhlenbeckActor(Actor):
    def __init__(self, obs_dim, act_dim, action_low, action_high, hidden_sizes, activation):
        super().__init__()
        self.action_space_low = torch.tensor(action_low)
        self.action_space_high = torch.tensor(action_high)
        self.action_ou_noise = OrnsteinUhlenbeckActionNoise(
    ...
    def _distribution(self, obs):
        mu = self.mu_net(obs)
        noise = self.action_ou_noise()
        noise = torch.tensor(noise).to('cuda:0')
        action_low = self.action_space_low.to('cuda:0')
        action_high = self.action_space_high.to('cuda:0')

        return torch.clip(mu + noise, action_low, action_high)
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space,
        ...
        # self.action_low = torch.tensor(action_space.low).to('cuda:0')
        # self.action_high = torch.tensor(action_space.high).to('cuda:0')

        # policy builder depends on action space
        if isinstance(action_space, Box):
            self.pi = MLPGaussianActor(obs_dim, action_space.shape[0], hidden_sizes, activation)
            # self.pi = MLPOrnsteinUhlenbeckActor(obs_dim, action_space.shape[0],
            #                                     action_space.low, action_space.high, hidden_sizes, activation)
