...
def ppo(env,
  ...
  (v): MLPCritic(
    ...
    for epoch in range(epochs):
        # action_ou_noise = OrnsteinUhlenbeckActionNoise(
        #     mean=np.zeros(act_dim[0]),
        #     sigma=0.06 * np.ones(act_dim[0]))
        for t in range(local_steps_per_epoch):
            a, v, logp = ac.step(torch.as_tensor(o, dtype=torch.float32))

            # # zhang: 对输出动作添加ou噪声
            # noise = action_ou_noise()
            # # noise = torch.tensor(noise).to('cuda:0')
            # # a = torch.clip(a + noise, env.action_space.low, env.action_space.high)
            # a += noise
            ...
            if terminal or epoch_ended:
                if epoch_ended and not terminal:
                    print('Warning: trajectory cut off by epoch at %d steps.' % ep_len,
                          flush=True)
                    # print("num_procs() = {}".format(num_procs()))
                    # print("t = {}".format(t))
                    # print("ep_len = {}".format(ep_len))
                    # print("local_steps_per_epoch = {}".format(local_steps_per_epoch-1))
