...
def ppo(env_fn, actor=nn.Module, critic=nn.Module, ac_kwargs=dict(), seed=0,
    ...
    if pretrain != None:
        ...
        ac_pi = actor(obs_dim[0], act_dim[0], hidden_sizes=[64, 64], activation=nn.Tanh,
                      pretrain=pretrain)  # env.observation_space, env.action_space, nn.ReLU)
    ...
    def compute_loss_pi(data):
        obs, act, adv, logp_old, hidden_h, hidden_c = data['obs'], data['act'], data['adv'], data['logp'], data[
            'hidden_h'], data['hidden_c']
    ...
    for epoch in range(epochs):
        hidden = (
        torch.zeros((1, 512), dtype=torch.float).to(device), torch.zeros((1, 512), dtype=torch.float).to(device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', type=str,
                        default='/root/lele/spinningup/spinningup/data/ppo_0715/ppo_0715_s0/pyt_save/model.pt')
